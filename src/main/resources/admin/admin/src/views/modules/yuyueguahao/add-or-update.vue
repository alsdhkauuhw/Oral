  ...
  <div class="addEdit-block">
    <el-form class="detail-form-content" ref="ruleForm" :model="ruleForm" :rules="rules" label-width="80px"
      ...
      <el-row>
        ...
        <!-- 更改 -->
        <el-col :span="12">
          <el-form-item class="input" v-if="type != 'info'" label="预约医生" prop="doctor">
          ...
          <div v-else>
            <el-form-item class="input" label="预约医生" prop="doctor">
              <el-option v-for="item in allDoctor" :key="item" :label="item" :value="item" />
            </el-form-item>
          </div>
        </el-col>
        <el-col :span="12">
          <el-form-item class="date" v-if="type != 'info'" label="预约就诊时间" prop="yuyuejiuzhenshijian">
            <el-date-picker value-format="yyyy-MM-dd HH:mm:ss" v-model="ruleForm.yuyuejiuzhenshijian" type="datetime"
              placeholder="预约就诊时间">
            </el-date-picker>
          </el-form-item>
          <div v-else>
            <el-form-item class="input" v-if="ruleForm.yuyuejiuzhenshijian" label="预约就诊时间" prop="yuyuejiuzhenshijian">
              <el-input v-model="ruleForm.yuyuejiuzhenshijian" placeholder="预约就诊时间" readonly></el-input>
            </el-form-item>
          </div>
        </el-col>
      </el-row>
      <el-row>
        <el-col :span="24">
          <el-form-item class="textarea" v-if="type != 'info'" label="备注" prop="beizhu">
            <el-input style="min-width: 200px; max-width: 600px;" type="textarea" :rows="8" placeholder="备注"
  ...
  methods: {
    ...
    init(id, type) {
      ...
      if (this.type == 'info' || this.type == 'else') {
        ...
        for (var o in obj) {
          ...
          if (o == 'yuyuejiuzhenshijian') {
            this.ruleForm.yuyuejiuzhenshijian = obj[o];
            this.ro.yuyuejiuzhenshijian = true;
            continue;
          }
          if (o == 'beizhu') {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `yuyueguahao/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yuyueguahaoCrossAddOrUpdateFlag = false;
                  this.parent.search();
