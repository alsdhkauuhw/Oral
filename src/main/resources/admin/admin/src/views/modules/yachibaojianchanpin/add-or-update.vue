          ...
          <el-form-item class="input" label="价格" prop="price">
              <el-input v-model="ruleForm.price" 
                placeholder="价格" readonly></el-input>
                                                                                ...
                                                                                <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="适用人群" prop="shiyongrenqun">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="适用人群"
                  v-model="ruleForm.shiyongrenqun" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.shiyongrenqun" label="适用人群" prop="shiyongrenqun">
                    <span>{{ruleForm.shiyongrenqun}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
                                                                                                        ...
                                                                                                        // 多级联动参数
                                                                                                                                    info(id) {
      this.$http({
        url: `yachibaojianchanpin/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `yachibaojianchanpin/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yachibaojianchanpinCrossAddOrUpdateFlag = false;
                  this.parent.search();
