  ...
  <div class="main-content">
    ...
    <div v-if="showFlag">
      <el-form :inline="true" :model="searchForm" class="form-content">
        <!-- 更改 -->
        <el-row :gutter="20" class="slt" v-if="!isAuth('yuyueguahao', '新增') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1"
          ...
          <el-form-item :label="contents.inputTitle == 1 ? '会员号' : ''">
            ...
            <el-input v-if="contents.inputIcon == 1 && contents.inputIconPosition == 2" suffix-icon="el-icon-search"
              v-model="searchForm.huiyuanhao" placeholder="会员号" clearable></el-input>
            <el-input v-if="contents.inputIcon == 0" v-model="searchForm.huiyuanhao" placeholder="医生"
              clearable></el-input>
          </el-form-item>
          <el-form-item>
      ...
      1 ? '查询' : '' }}</el-button>
        ...
        <el-row class="ad"
          ...
          <el-form-item>
            <el-button
              v-if="isAuth('yuyueguahao', '新增') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1"
              type="success" icon="el-icon-plus" @click="addOrUpdateHandler()">{{ contents.btnAdAllFont == 1 ? '新增' : ''
              }}</el-button>
            <el-button
              v-if="isAuth('yuyueguahao', '新增') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 2"
              type="success" @click="addOrUpdateHandler()">{{ contents.btnAdAllFont == 1 ? '新增' : '' }}<i
                class="el-icon-plus el-icon--right" /></el-button>
            <el-button v-if="isAuth('yuyueguahao', '新增') && contents.btnAdAllIcon == 0" type="success"
              @click="addOrUpdateHandler()">{{ contents.btnAdAllFont == 1 ? '新增' : '' }}</el-button>
            <el-button
              v-if="isAuth('yuyueguahao', '删除') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1 && contents.tableSelection"
              :disabled="dataListSelections.length <= 0" type="danger" icon="el-icon-delete" @click="deleteHandler()">{{
      contents.btnAdAllFont == 1 ? '删除' : '' }}</el-button>
            <el-button
              v-if="isAuth('yuyueguahao', '删除') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 2 && contents.tableSelection"
      ...
      == 1 ? '删除' : '' }}<i class="el-icon-delete el-icon--right" /></el-button>
            <el-button v-if="isAuth('yuyueguahao', '删除') && contents.btnAdAllIcon == 0 && contents.tableSelection"
              :disabled="dataListSelections.length <= 0" type="danger" @click="deleteHandler()">{{ contents.btnAdAllFont
      == 1 ? '删除' : '' }}</el-button>
      ...
      <div class="table-content">
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
          ...
          v-if="isAuth('yuyueguahao', '查看')" :data="dataList" v-loading="dataListLoading"
          ...
          <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="yuyuejiuzhenshijian"
            header-align="center" label="预约就诊时间">
            <template slot-scope="scope">
              {{ scope.row.yuyuejiuzhenshijian }}
            </template>
          </el-table-column>
          <!-- 更改 -->
          <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="doctor"
            header-align="center" label="预约医生">
            <template slot-scope="scope">
              {{ scope.row.doctor }}
            </template>
          </el-table-column>
          ...
          <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="shhf"
            header-align="center" label="审核回复">
          </el-table-column>
          <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="sfsh"
            header-align="center" label="审核状态">
            <template slot-scope="scope">
              <span style="margin-right:10px">{{ scope.row.sfsh == '是' ? '通过' : '未通过' }}</span>
            </template>
          </el-table-column>
          <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
            v-if="isAuth('yuyueguahao', '审核')" prop="sfsh" header-align="center" label="审核">
            <template slot-scope="scope">
          ...
          <el-table-column width="300" :align="contents.tableAlign" header-align="center" label="操作">
            <template slot-scope="scope">
              <el-button
                v-if="isAuth('yuyueguahao', '查看') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
      ...
      contents.tableBtnFont == 1 ? '详情' : '' }}</el-button>
              <el-button
                v-if="isAuth('yuyueguahao', '查看') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
      ...
      1 ? '详情' : '' }}<i class="el-icon-tickets el-icon--right" /></el-button>
              <el-button v-if="isAuth('yuyueguahao', '查看') && contents.tableBtnIcon == 0" type="success" size="mini"
                @click="addOrUpdateHandler(scope.row.id, 'info')">{{ contents.tableBtnFont == 1 ? '详情' : '' }}</el-button>
              <el-button
                v-if="isAuth('yuyueguahao', '修改') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
                type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">{{
      contents.tableBtnFont == 1 ? '修改' : '' }}</el-button>
              <el-button
                v-if="isAuth('yuyueguahao', '修改') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
      ...
      1 ? '修改' : '' }}<i class="el-icon-edit el-icon--right" /></el-button>
              <el-button v-if="isAuth('yuyueguahao', '修改') && contents.tableBtnIcon == 0" type="primary" size="mini"
                @click="addOrUpdateHandler(scope.row.id)">{{ contents.tableBtnFont == 1 ? '修改' : '' }}</el-button>
              <el-button
                v-if="isAuth('yuyueguahao', '删除') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1"
                type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">{{
      contents.tableBtnFont == 1 ? '删除' : '' }}</el-button>
              <el-button
                v-if="isAuth('yuyueguahao', '删除') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2"
              ...
              <el-button v-if="isAuth('yuyueguahao', '删除') && contents.tableBtnIcon == 0" type="danger" size="mini"
...
export default {
  ...
  methods: {
    ...
    shDialog(row) {
      ...
      if (row) {
        this.shForm = {
          huiyuanhao: row.huiyuanhao,
          xingming: row.xingming,
          yuyuejiuzhenshijian: row.yuyuejiuzhenshijian,
          beizhu: row.beizhu,
          sfsh: row.sfsh,
          shhf: row.shhf,
          id: row.id,
