...
export default {
  ...
  methods: {
	    ...
	    this.ro.reply = true;
            continue;
                                        ...
                                        // 多级联动参数
                                                    info(id) {
      this.$http({
        url: `discussyachibaojianchanpin/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `discussyachibaojianchanpin/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.discussyachibaojianchanpinCrossAddOrUpdateFlag = false;
                  this.parent.search();
